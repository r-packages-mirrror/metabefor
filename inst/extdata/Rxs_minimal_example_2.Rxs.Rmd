```{r rxs-setup-chunk-jkTF02, echo=FALSE, results='hide'}
```
...
##############################################################################
  ...
  ############################################################################
    ...
    ### 
    ### This is already available in the screening database; a QURID
    ### was added to every record. We will use this to automatically
    ...
    ### the screening we look for people (e.g. colleagues or
    ### students) who can read a certain language. If not the article
    ### will be excluded. Use ISO 639-1 to extract this (see
    ### https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes). If an
    ### article is written in a language that is not represented in
    ### ISO 639-1, extract using ISO 639-2 instead.
    ### 
    ##########################################################################
    ...
    ### AUTHORS
    ### 
    ### This container entity holds the information about the
    ### authors, in repeated individual container entities per author
    ### that each hold two extractable entities.
    ### 
    ##########################################################################
        
    
      ########################################################################
      #################################### START: sourceAuthor (REPEATING) ###
      ########################################################################
      currentEntity <- currentEntity$AddChild('sourceAuthor__1__');
      currentEntity[['value']] <-
      ########################################################################
      ### 
      ### AUTHOR
      ### 
      ### This container entity holds the information about one
      ### author, in two extractable entities (name and ORCID).
      ### 
      ########################################################################
          
          list(sourceAuthorIdentifier = NULL,  ### Author identifier: A unique identifier for this author in this source. Most likely, this author's last name suffices. If multiple authors share a last name, number them (e.g. "smith1", "smith2", etc). [Examples: "example1"; "another_identifier_example"; "finalExample"] [Value description: A single character value that is used as an identifier and so is always mandatory and can only contain a-z, A-Z, 0-9, and underscores, and must start with a letter.]
               sourceAuthorName = NULL,        ### Author Name: This author's name. The name of this author, as provided on the article. If no name is present, extract this as NA. [Examples: "example"; "another example"] [Value description: A single character value]
               sourceAuthorORCID = NULL);      ### NA
          
      ########################################################################
      currentEntity[['validation']] <- list(`sourceAuthorIdentifier` = expression(grepl("[a-zA-Z][a-zA-Z0-9_]*", VALUE)),
                                            `sourceAuthorName` = expression((length(VALUE) == 1) && (is.character(VALUE)) || all(is.na(VALUE))),
                                            `sourceAuthorORCID` = expression(grepl("([0-9][0-9][0-9][0-9]-){3}[0-9][0-9][0-9][0-9]", VALUE)));
      currentEntity$name <- metabefor::nodeName(currentEntity$value[[1]], "sourceAuthor__1__");
      currentEntity <- currentEntity$parent;
      ########################################################################
      ###################################### END: sourceAuthor (REPEATING) ###
      ########################################################################
  ...
  ############################################################################
    ...
    ##########################################################################
    ############################################ START: sample (REPEATING) ###
    ##########################################################################
    currentEntity <- currentEntity$AddChild('sample__1__');
    ##########################################################################
    ### 
    ### SAMPLE
    ### 
    ...
    ##########################################################################
      ...
      ########################################################################
          ...
          list(variableIdentifier = NULL,  ### NA
               var_UCID = NULL,            ### NA
               measurementLevel = NULL);   ### NA
          
      ########################################################################
      currentEntity[['validation']] <- list(`variableIdentifier` = expression(grepl("[a-zA-Z][a-zA-Z0-9_]*", VALUE)),
                                            `var_UCID` = expression((length(VALUE) == 1) && (is.character(VALUE)) || all(is.na(VALUE))),
  ...
  ############################################################################
    ...
    ##########################################################################
      ...
      ########################################################################
          ...
          list(associationIdentifier = NULL,         ### NA
...
```{r rxs-template-specification-chunk-YLebrX, echo=FALSE, results='hide'}
```

```{r rxs-validation-chunk-CiB9jM, results='asis'}
...
```{r rxs-show-extracted-data-chunk-2GfowV}
if ((!(exists('parsingMultipleRxsFiles'))) || (!parsingMultipleRxsFiles)) {  metabefor::rxs_partial(rxsObject); }
```

```{r rxs-setup-chunk-jkTF02, include=FALSE, messages=FALSE}
...
```{r rxs-template-specification-chunk-YLebrX, echo=FALSE}

rxsTemplateSpec <- 
  list(entities = structure(list(title = c("General", "QURID", 
  "Language", "Authors", "Author", "Author identifier", "Author Name", 
  "Author ORCID", "Methods", "Sample", "Sample Identifier", "Sample size", 
  "Sampling Strategy", "Method", "Variables", "Variable", "Variable Identifier", 
  "Identifier of the construct", "Measurement Level", "Results", 
  "Associations", "Association", "Association Identifier", "Variable Identifier for variable 1", 
  "Variable Identifier for variable 2", "Pearson's r", "Student's t"
  ), description = c("General information", "Quasi Unique Identifier Record Identifier (QURID).", 
  "The written language of the article.", "This container entity holds the information about the authors, in repeated individual container entities per author that each hold two extractable entities.", 
  "This container entity holds the information about one author, in two extractable entities (name and ORCID).", 
  "A unique identifier for this author in this source.", "This author's name.", 
  ...
  "The unique identifier for this sample. This is used to refer to the sample elsewhere in the extraction.", 
  ...
  "The unique identifier for this variable. This is used to refer to the variable elsewhere in the extraction.", 
  "A unique identifier of the construct that this variable pretains to. This is either the UCID of the construct, if this is a determinant or proxy; or an identifier from the list provided in the 'variable identifiers' list in the extraction instructions.", 
  ...
  "A unique identifier for this association. This is used to refer to the association elsewhere in the extraction or during the synthesis.", 
  ...
  ), instructions = c(NA, "This is already available in the screening database; a QURID was added to every record. We will use this to automatically import bibliographic information available in that file, such as title, keywords, potentially abstract, etc.", 
  ...
  NA, NA, "Most likely, this author's last name suffices. If multiple authors share a last name, number them (e.g. \"smith1\", \"smith2\", etc).", 
  "The name of this author, as provided on the article. If no name is present, extract this as NA.", 
  NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 
  NA, NA, NA, NA), identifier = c("general", "qurid", "language", 
  "sourceAuthors", "sourceAuthor", "sourceAuthorIdentifier", "sourceAuthorName", 
  "sourceAuthorORCID", "methods", "sample", NA, "sampleSize", "samplingStrategy", 
  "method", "variables", "variable", "variableIdentifier", "var_UCID", 
  "measurementLevel", "results", "associations", "association", 
  "associationIdentifier", "varId1", "varId2", "r", "t"), valueTemplate = c(NA, 
  "string.identifier", "string", NA, NA, "string.identifier", "string", 
  "ORCID", NA, NA, NA, "integer", "string", "categorical", NA, 
  NA, "string.identifier", "string", "categorical", NA, NA, NA, 
  "string.identifier", "string.entityRef.mandatory", "string.entityRef.mandatory", 
  ...
  NA, NA, NA, NA, NA, NA, NA, NA), parent = c(NA, "general", "general", 
  "general", "sourceAuthors", "sourceAuthor", "sourceAuthor", "sourceAuthor", 
  NA, "methods", NA, "sample", "sample", "methods", "methods", 
  "variables", "variable", "variable", "variable", NA, "results", 
  "associations", "association", "association", "association", 
  "association", "association"), ontologyId = c(NA, NA, NA, NA, 
  NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 
  NA, NA, NA, NA, NA, "http://purl.obolibrary.org/obo/STATO_0000280", 
  "http://purl.obolibrary.org/obo/STATO_0000176"), default = c(NA_character_, 
  ...
  NA_character_, NA_character_, NA_character_, NA_character_), 
      list = c(NA, NA, NA, NA, "TRUE", NA, NA, NA, NA, NA, NA, 
      NA, NA, NA, NA, "TRUE", NA, NA, NA, NA, NA, "TRUE", NA, NA, 
      NA, NA, NA), identifying = c(NA, NA, NA, NA, "TRUE", NA, 
      NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, "TRUE", NA, NA, NA, 
      NA, NA, "TRUE", NA, NA, NA, NA), repeating = c(NA, NA, NA, 
      NA, "TRUE", NA, NA, NA, NA, "TRUE", NA, NA, NA, NA, NA, "TRUE", 
      NA, NA, NA, NA, NA, "TRUE", NA, NA, NA, NA, NA), module = c(NA_character_, 
      NA_character_, NA_character_, NA_character_, NA_character_, 
      NA_character_, NA_character_, NA_character_, NA_character_, 
      NA_character_, NA_character_, NA_character_, NA_character_, 
      NA_character_, NA_character_, NA_character_, NA_character_, 
      NA_character_, NA_character_, NA_character_, NA_character_, 
      NA_character_, NA_character_, NA_character_, NA_character_, 
      NA_character_, NA_character_), entityRef = c(NA, NA, NA, 
      NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 
      NA, NA, NA, NA, NA, "variableIdentifier", "variableIdentifier", 
      ...
      NA_character_, NA_character_, NA_character_, NA_character_, 
      NA_character_, NA_character_, NA_character_, NA_character_
      )), row.names = c(NA, -27L), class = "data.frame"), valueTemplates = structure(list(
      ...
      "matrix.crosstab", "string.identifier", "ORCID"), description = c("Any valid number", 
      ...
      NA, NA, NA, NA, NA, NA, NA), default = c("NULL", "NULL", 
      "NULL", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL", 
      "\"Enter name here (mandatory)\"", "\"enter <<entityRef>> here\"", 
      "NULL", "NULL", "NULL", "NULL", "NULL"), examples = c("2.3 || 643.2", 
      "c(23.43, 62) || 52.2 || c(76, 12.56, 42)", "30 || 8762", 
      ...
      "\"0000-0003-2814-9483\""), validation = c("(length(VALUE) == 1) && is.na(VALUE) || (is.numeric(VALUE))", 
      ...
      "all(is.na(VALUE)) || (VALUE %in% <<validValues>>)", "all(is.na(VALUE)) || (VALUE %in% <<validValues>>)", 
      ...
      "grepl(\"([0-9][0-9][0-9][0-9]-){3}[0-9][0-9][0-9][0-9]\", VALUE)"
      ), error = c("NAME is not numeric, or contains more than one value, but has value: VALUE", 
      "NAME is not a numeric vector, but has value: VALUE", "NAME is not a single whole number, but has value: VALUE", 
  ...
  -1L), class = "data.frame"), instructionSheet = structure(list(
      ...
      "One more heading"), description = c("These instructions are for extractors using the minimal rxs example.", 
      ...
      )), row.names = c(NA, -4L), class = "data.frame"), textsSheet = structure(list(
      textIdentifier = c("openingRemarks", "closingRemarks", "extractionOverview_list_intro", 
      "extractionOverview_compact_intro"), content = c("Welcome to the R Extraction Script (.rxs.Rmd file) for this source!\n\nYou can now start extracting. If you haven't yet studied the extractor instructions, please do so first. If you're all set, good luck!", 
