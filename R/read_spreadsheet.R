...
read_spreadsheet <- function(x,
  ...
  gSheetId_extractionRegex <- metabefor::opts$get("gSheetId_extractionRegex");
  gSheetId_to_exportLink <- metabefor::opts$get("gSheetId_to_exportLink");
  ...
  if (length(x) == 0) {
    ...
    stop("As `x`, you must pass a character value (i.e. a single string). ",
         "Instead, you passed a character vector without any elements.");
    
  } else if (length(x) > 1) {

    msg("As `x`, you specified ", length(x), " character values. ",
        "Recursively calling myself on each element.\n",
        silent = silent);
    ...
    res <- lapply(x,
                  ...
                  flattenSingleDf = flattenSingleDf,
                  xlsxPkg = xlsxPkg,
                  failQuietly = failQuietly,
                  silent = silent);
    
    msg("Combined results of recursively calling myself on each element; ",
  ...
  if (isTRUE(downloaded) && (!is.null(localBackup))) {

    msg("You passed a value for `localBackup` (", localBackup,
        "), so attempting to save a local backup.\n",
        silent = silent);

    if (exists("downloadResult") &&
        !is.null(downloadResult) &&
        (downloadResult == 0)) {
      ...
      if ((length(localBackup) == 1) &&
          (grepl("xls",
                 tools::file_ext(localBackup),
                 ignore.case = TRUE))) {
        
        msg("Storing the local backup in a spreadsheet.\n",
            silent = silent);
      ...
      } else if (!is.null(names(localBackup)) &&
                 (all(grepl("csv$",
                            tools::file_ext(localBackup),
                            ignore.case = TRUE)))) {
          
        msg("Storing the local backup in a set of `csv` files.\n",
            silent = silent);
      
        for (currentSheetName in names(localBackup)) {
          if (!is.null(localBackup[currentSheetName])) {
            if (dir.exists(dirname(localBackup[currentSheetName]))) {
              ### Sometimes, for some odd reason, columns have
              ### the 'list' class; convert those to character.
              utils::write.csv(localBackup[currentSheetName],
                               row.names=FALSE,
                               purgeListsFromDf(
                                 res[[currentSheetName]])
                               );
            } else {
              stop("You specified '", localBackup[currentSheetName],
                   "' as a path to save a local backup to, but the ",
        ...
        stop("As local backups, I can only write to a single `.xlsx` file, ",
             "or so a set of `.csv` files. You passed an extension I don't ",
             "know (", vecTxtQ(tools::file_ext(localBackup$entities)), ").");
      }
