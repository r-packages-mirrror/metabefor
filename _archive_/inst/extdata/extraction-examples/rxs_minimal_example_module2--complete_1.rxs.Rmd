...
```{r rxs-setup-chunk-6Q5iMa, echo=FALSE, results='hide'}
```
...
################################################################################
  ...
  ##############################################################################
    ...
    ##################################################### START: sourceTitle ###
    ############################################################################
    currentEntity <- currentEntity$AddChild('modeInfo');
    currentEntity[['value']] <-
    ############################################################################
    ### 
    ### TITLE
  ...
  ##############################################################################
    ...
    ############################################################################
      ...
      ### 
      ##########################################################################
      currentEntity[['validation']] <- expression(is.na(VALUE) || (is.numeric(VALUE) && (VALUE%%1==0) && (length(VALUE) == 1)));
      currentEntity <- currentEntity$parent;
      ##########################################################################
      ###################################################### END: sampleSize ###
      ##########################################################################
      ...
      ##########################################################################
          ...
          "Wandering the hillsides and collecting sheep"
    ...
    ############################################################################
      ...
      ##########################################################################
          ...
          list(variableIdentifier = "chalk",  ### Variable Identifier: The unique identifier for this variable [Examples: "example1"; "another_identifier_example"; "finalExample"] [Value description: A single character value that is used as an identifier and so is always mandatory and can only contain a-z, A-Z, 0-9, and underscores, and must start with a letter.]
               measurementLevel = "Continuous");   ### Results: The results from this study. [Examples: c("Nominal", "Ordinal", "Continuous")] [Value description: A string that has to exactly match one of the values specified in the "values" column of the Coding sheet]
      ...
      ##########################################################################
          ...
          list(variableIdentifier = "witches",  ### Variable Identifier: The unique identifier for this variable [Examples: "example1"; "another_identifier_example"; "finalExample"] [Value description: A single character value that is used as an identifier and so is always mandatory and can only contain a-z, A-Z, 0-9, and underscores, and must start with a letter.]
...
rxsObject$rxsMetadata <- list(rxsVersion='0.3.0', moduleId="module2", id=uniqueSourceIdentifier);
...
```{r rxs-show-extracted-data-chunk-mPgZVr}
metabefor::rxs_partial(rxsObject);
```

```{r rxs-validation-chunk-F6OUfy, results='asis'}
...
```{r rxs-setup-chunk-6Q5iMa, include=FALSE, messages=FALSE}
...
```{r rxs-template-specification-chunk-rzp1mY, echo=FALSE}
